...
#                                                                                         #
# Descrição: Testes com dados do dataset ant nest.                                        #
#            (Fonte 1: http://www.ulb.ac.be/di/labo/data/NO5/NO5.zip)                     #
#            (Fonte 2: http://www.ulb.ac.be/di/labo/datasets.html)                        #
#                                                                                         #
#            Cabeçalho utilizado no dataset (TMote Sky):                                  #
#            timestamp nodeid seqnumber itemperature ivoltage                             #
#            humidity temperature PAR TSR                                                 #
#                                                                                         #
#                                                                                         #
###########################################################################################
...
TL = 2              # Limiar de temperatura

ZI = 2.990          # Controle de influência zero

GM_LEN =100          # Tamanho da janela do GM(1,1)

JI = 500           # Janela inicial de observação

JF = 800           # Janela final de observação
...
obter_dados_dataset = function (node)                   {
  ...
  nodeset = dataset[which(dataset$nodeid == node),]
...
obter_confianca_node = function (nodeVoltage)           {
  ...
  for (i in 1:length(nodeVoltage))
  {
    ...
    {
      if(nodeVoltage[i] >= 2.5)
      {
        out[i] = 0.075 
      }
      else
      {
        if(nodeVoltage[i] >= 2.2)
        {
          out[i] = 0.0075 #0.5
        }
        else
...
obter_confianca_node_orig = function (nodeVoltage)      {
...
predict_gm_enhanced_2 = function (X0, win_size)         {
  ...
  if (win_size > 0)
  {
    while ((win_end + win_size) < length(X0)) {
      ...
      out[win_start:win_end] = tail(predict_gm_model (c(tail(out,3), X0[win_start:win_end]),-1),win_size)
...
x = seq(0,nrow(y)-1)
x = x * 0.3                   # Transforma a escala em segundos

r = q <- sd (y$temperature)

z = filtro_kalman_novo_3 (y$temperature, y$ivoltage, r, q)
w = filtro_kalman_classico (y$temperature, r, q)
u = predict_gm_enhanced_2 (y$temperature, as.integer(length(y$temperature)/GM_LEN))

plot   (x[JI:JF], y$temperature[JI:JF], type="l",col="black",xlab="Tempo (Ks)",ylab="Temperatura (ºC)")
